...
of CCNxKE.  CCNxKE also supports the concept of a MoveToken, which allows then authenticating
...
one way to use the traffic secret to carry out communications.  
Annex A also sketches out an example CCNx protocol for exchangig encrypted messages,
...
The use of CCNxKE does not, in short, require the use of a TLS-like transport protocol.
...
data security, such as Content Object provenance via signatures, group encryption of cached objects,
...
normal CCNx packets with normal CCNx attributes such as signatures and hash chains.
...
There is currently no standard way for CCNx peers to excahnge emphemeral, forward
secure keys, thus this RFC specifies the standard mechanism that should be used
buy all CCNx peers for such keys. CCNxKE
